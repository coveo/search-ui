...
module Coveo {
  describe('ResultsPerPage', function () {
    ...
    beforeEach(function () {
      ...
      test.env.queryController.options.page = 1;
    })
    ...
    describe('analytics', function () {
      it('should log the proper event when changing the number of results per page', function () {
        test.cmp.setResultsPerPage(50);
        expect(test.env.usageAnalytics.logCustomEvent).toHaveBeenCalledWith(analyticsActionCauseList.pagerResize, { pagerNumber: 1, currentResultsPerPage: 50 }, test.cmp.element);
      })
    })
