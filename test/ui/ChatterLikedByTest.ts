...
module Coveo {
  ...
  describe('ChatterLikedBy', () => {
    ...
    it('should behave correctly with a single like', () => {
      ...
      expect($$(test.cmp.element).find('a').getAttribute('href')).toEqual(ChatterUtils.buildURI(result.clickUri, result.raw.sffeeditemid, result.raw.sflikedbyid));
    ...
    it('should behave correctly with multiple likes', () => {
      ...
      expect($$(test.cmp.element).find('a').getAttribute('href')).toEqual(ChatterUtils.buildURI(result.clickUri, result.raw.sffeeditemid, 'LikeId1'));
      ...
      expect($$(test.cmp.element).findAll('a')[1].getAttribute('href')).toEqual(ChatterUtils.buildURI(result.clickUri, result.raw.sffeeditemid, 'LikeId2'));
    ...
    it('Test behaviour when nbLikeToRender is set to 0 with multiple likes', () => {
      ...
      expect($$(test.cmp.element).findAll('a')[1].getAttribute('href')).toContain(ChatterUtils.buildURI(result.clickUri, result.raw.sffeeditemid, 'LikeId2'));
