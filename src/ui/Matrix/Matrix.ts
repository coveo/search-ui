...
module Coveo {
  ...
  export class Matrix extends Component {
    ...
    private instantiateTemplate(result: IQueryResult): HTMLElement {
      var content = this.options.previewTemplate.instantiateToElement(result, false);
    ...
    private createPreviewQuery(rowNumber: number, columnNumber: number): IQuery {
      var rowFieldExpression = '(' + QueryUtils.buildFieldExpression(this.options.rowField, '=', [this.getRowValue(rowNumber)]) + ')';
      var columnFieldExpression = '(' + QueryUtils.buildFieldExpression(this.options.columnField, '=', [this.getColumnValue(columnNumber)]) + ')';
