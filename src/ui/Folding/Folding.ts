...
module Coveo {
  ...
  export class Folding extends Component {
    ...
    public static defaultGetResult(result: IQueryResult) {
      var results: IQueryResult[] = result.childResults || [];
    ...
    private handlepreprocessResults(data: IPreprocessResultsEventArgs) {
      ...
      var getResult: (result: IQueryResult) => IQueryResult = this.options.getResult || Folding.defaultGetResult;
    ...
    private moreResults(result: IQueryResult, originalQuery: IQuery): Promise<IQueryResult[]> {
      ...
      var fieldValue = Utils.getFieldValue(result, this.options.field);
    ...
    private handlePreprocessMoreResults(queryResults: IQueryResults) {
      var getResults: (results: IQueryResult[]) => IQueryResult[] = this.options.getMoreResults || Folding.defaultGetMoreResults;
  ...
  Coveo.Initialization.registerAutoCreateComponent(Folding);
}
