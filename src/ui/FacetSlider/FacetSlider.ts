...
module Coveo {
  ...
  export class FacetSlider extends Component {
    ...
    private buildSlider() {
      ...
      $$(sliderDiv).on(SliderEvents.endSlide, (e: MouseEvent, args: EndSlideEventArgs) => {
        this.handleEndSlide(args);
      });
      $$(sliderDiv).on(SliderEvents.duringSlide, (e: MouseEvent, args: DuringSlideEventArgs) => {
        this.handleDuringSlide(args);
      });
      if (this.hasAGraph()) {
        $$(sliderDiv).on(SliderEvents.graphValueSelected, (e: MouseEvent, args: GraphValueSelectedArgs) => {
    ...
    private handleEndSlide(args: EndSlideEventArgs) {
    ...
    private handleDuringSlide(args: DuringSlideEventArgs) {
      var values = args.slider.getValues();
      this.startOfSlider = values[0];
      this.endOfSlider = values[1];
      this.slider.setValues([this.startOfSlider, this.endOfSlider]);
    ...
    private handleGraphValueSelected(args: GraphValueSelectedArgs) {
    ...
    private updateQueryState(values = this.slider.getValues(), silent = false) {
      ...
      var model: number[] = this.queryStateModel.get(this.rangeQueryStateAttribute);
    ...
    private renderToSliderGraph(data: IQuerySuccessEventArgs) {
      ...
      var graphData: ISliderGraphData[];
