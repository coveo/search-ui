  ...
  export class Triggers extends Component {
    ...
    private handleProcessNewQueryResults(data: IQuerySuccessEventArgs) {
      ...
      this.executeTriggers(data.results.triggers, 'execute', (trigger: ITriggerExecute) => {
        try {
          var func: Function = this._window['' + trigger.content.name]
          if (typeof func === 'function') {
            var params = _.object(_.map(trigger.content.params, (value, index) => { return ["param" + (index + 1), value] }));
    ...
    private executeTriggers(Triggers: ITrigger<any>[], type: string, func: (trigger: ITrigger<any>) => any, single: boolean = false) {
      var TriggersOfType = _.filter(Triggers, (trigger: ITrigger<any>) => { return trigger.type == type });
