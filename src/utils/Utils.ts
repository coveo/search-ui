import {IQueryResult} from '../rest/QueryResult';
import {Assert} from '../misc/Assert';
import _ = require('underscore');

export function isUndefined(obj: any): boolean {
  return typeof obj == 'undefined';
}

export function isNull(obj: any): boolean {
  return obj === null;
}

export function isNullOrUndefined(obj: any): boolean {
  return isUndefined(obj) || isNull(obj);
}

export function exists(obj: any): boolean {
  return !isNullOrUndefined(obj);
}

export function toNotNullString(str: string): string {
  return _.isString(str) ? str : "";
}

export function anyTypeToString(value: any): string {
  return value ? value.toString() : '';
}

export function isNullOrEmptyString(str: string): boolean {
  return isNullOrUndefined(str) || !isNonEmptyString(str);
}

export function isNonEmptyString(str: string): boolean {
  return _.isString(str) && str !== '';
}

export function isEmptyString(str: string): boolean {
  return !isNonEmptyString(str);
}

export function stringStartsWith(str: string, startWith: string): boolean {
  return str.slice(0, startWith.length) == startWith;
}

export function isNonEmptyArray(obj: any): boolean {
  return _.isArray(obj) && obj.length > 0;
}

export function isEmptyArray(obj: any): boolean {
  return !isNonEmptyArray(obj);
}

export function isHtmlElement(obj: any): boolean {
  if (window["HTMLElement"] != undefined) {
    return obj instanceof HTMLElement;
  } else { //IE 8 FIX
    return obj && obj.nodeType && obj.nodeType == 1
  }
}

export function parseIntIfNotUndefined(str: string): number {
  if (isNonEmptyString(str)) {
    return parseInt(str, 10);
  } else {
    return undefined;
  }
}

export function parseFloatIfNotUndefined(str: string): number {
  if (isNonEmptyString(str)) {
    return parseFloat(str);
  } else {
    return undefined;
  }
}

export function round(num: number, decimals: number) {
  return Math.round(num * Math.pow(10, decimals)) / Math.pow(10, decimals);
}

export function parseBooleanIfNotUndefined(str: string): boolean {
  if (isNonEmptyString(str)) {
    switch (str.toLowerCase()) {
      case 'true':
      case '1':
      case 'yes':
        return true;
      case 'false':
      case '0':
      case 'no':
        return false;
      default:
        return undefined;
    }
  } else {
    return undefined;
  }
}

export function trim(value: string): string {
  if (value == null) {
    return null;
  }
  return value.replace(/^\s+|\s+$/g, '');
}

export function encodeHTMLEntities(rawStr: string) {
  var ret = [];
  for (var i = rawStr.length - 1; i >= 0; i--) {
    if (/^[a-z0-9]/i.test(rawStr[i])) {
      ret.unshift(rawStr[i]);
    } else {
      ret.unshift(['&#', rawStr.charCodeAt(i), ';'].join(''));
    }
  }
  return ret.join('');
}

export function decodeHTMLEntities(rawString: string) {
  return rawString.replace(/&#(\d+);/g, function (match, dec) {
    return String.fromCharCode(dec);
  });
}

export function arrayEqual(array1: any[], array2: any[], sameOrder: boolean = true): boolean {
  if (sameOrder) {
    return _.isEqual(array1, array2);
  }
  else {
    var arrays = [array1, array2]
    return _.all(arrays, (array: any[]) => {
      return array.length == arrays[0].length && _.difference(array, arrays[0]).length == 0;
    });
  }
}

export function objectEqual(obj1: Object, obj2: Object): boolean {
  return _.isEqual(obj1, obj2);
}

var isCoveoFieldRegex = /^@[a-zA-Z0-9_\.]+$/
export function isCoveoField(field: string): boolean {
  return isCoveoFieldRegex.test(field);
}

export function escapeRegexCharacter(str: string) {
  var ret = str.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&");
  return ret;
}

export function getCaseInsensitiveProperty(object: {}, name: string): any {
  // First try using a fast case-sensitive lookup
  var value = object[name];

  // Then try a fast case-sensitive lookup with lowercase name
  if (value == null) {
    var lowerCaseName = name.toLowerCase();
    value = object[lowerCaseName];

    // Then try a slow scanning of all the properties
    if (value == null) {
      var matchingKey = _.find(_.keys(object), (key: string) => key.toLowerCase() == lowerCaseName);
      if (matchingKey != null) {
        value = object[matchingKey];
      }
    }
  }

  return value;
}

export function getFieldValue(result: IQueryResult, name: string): any {
  // Be as forgiving as possible about the field name, since we expect
  // user provided values.

  if (name == null) {
    return undefined;
  }

  name = trim(name);
  if (name[0] == '@') {
    name = name.substr(1);
  }

  if (name == '') {
    return undefined;
  }

  // At this point the name should be well formed
  Assert.check(isCoveoField('@' + name));

  // Handle namespace field values of the form sf.Foo.Bar
  var parts = name.split('.').reverse();
  var obj = result.raw;
  while (parts.length > 1) {
    obj = getCaseInsensitiveProperty(obj, parts.pop());
    if (isUndefined(obj)) {
      return undefined;
    }
  }

  var value = getCaseInsensitiveProperty(obj, parts[0]);

  // If still nothing, look at the root of the result
  if (value == null) {
    value = getCaseInsensitiveProperty(result, name);
  }

  return value;
}

export function throttle(func, wait, options: { leading?: boolean; trailing?: boolean; } = {}, context?, args?) {
  var result;
  var timeout: number = null;
  var previous = 0;
  var later = function () {
    previous = options.leading === false ? 0 : new Date().getTime();
    timeout = null;
    result = func.apply(context, args);
  };
  return function () {
    var now = new Date().getTime();
    if (!previous && options.leading === false) {
      previous = now;
    }
    var remaining = wait - (now - previous);
    context = this;
    args = arguments;
    if (remaining <= 0) {
      clearTimeout(timeout);
      timeout = null;
      previous = now;
      result = func.apply(context, args);
    } else if (!timeout && options.trailing !== false) {
      timeout = setTimeout(later, remaining);
    }
    return result;
  };
}

export function extendDeep(target, src): {} {
  var isArray = _.isArray(src)
  var toReturn = isArray && [] || {}

  if (isArray) {
    target = target || []
    toReturn = toReturn["concat"](target)
    _.each(src, (e, i, obj) => {
      if (typeof target[i] === 'undefined') {
        toReturn[i] = <any>e
      } else if (typeof e === 'object') {
        toReturn[i] = extendDeep(target[i], e)
      } else {
        if (target.indexOf(e) === -1) {
          toReturn["push"](e)
        }
      }
    })
  } else {
    if (target && typeof target === 'object') {
      _.each(_.keys(target), (key) => {
        toReturn[key] = target[key]
      })
    }
    _.each(_.keys(src), (key) => {
      if (typeof src[key] !== 'object' || !src[key]) {
        toReturn[key] = src[key]
      } else {
        if (!target[key]) {
          toReturn[key] = src[key]
        } else {
          toReturn[key] = extendDeep(target[key], src[key])
        }
      }
    })
  }

  return toReturn
}

export function getQueryStringValue(key, queryString = window.location.search) {
  return queryString.replace(new RegExp("^(?:.*[&\\?]" + key.replace(/[\.\+\*]/g, "\\$&") + "(?:\\=([^&]*))?)?.*$", "i"), "$1");
}

export function isValidUrl(str: string): boolean {
  var regexp = /(ftp|http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/
  return regexp.test(str);
}

export function debounce(func: Function, wait: number) {
  var timeout: number;
  var stackTraceTimeout: number;
  return function (...args: any[]) {
    if (timeout == null) {
      timeout = setTimeout(() => {
        timeout = null;
      }, wait);
      stackTraceTimeout = setTimeout(() => {
        func.apply(this, args);
        stackTraceTimeout = null;
      });
    } else if (stackTraceTimeout == null) {
      clearTimeout(timeout);
      timeout = setTimeout(() => {
        func.apply(this, args);
        timeout = null;
      }, wait);
    }
  }
}

export function readCookie(name: string) {
  var nameEQ = name + "=";
  var ca = document.cookie.split(';');
  for (var i = 0; i < ca.length; i++) {
    var c = ca[i];
    while (c.charAt(0) == ' ') {
      c = c.substring(1, c.length);
    }
    if (c.indexOf(nameEQ) == 0) {
      return c.substring(nameEQ.length, c.length);
    }
  }
  return null;
}

export function toDashCase(camelCased: string) {
  return camelCased.replace(/([a-z][A-Z])/g, (g) => g[0] + '-' + g[1].toLowerCase());
}
