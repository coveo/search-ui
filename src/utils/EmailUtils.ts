...
module Coveo {
  export class EmailUtils {
    static splitSemicolonSeparatedListOfEmailAddresses(addresses: string): string[] {
      ...
      return _.filter(addressesAsList, (s: string) => { return Coveo.Utils.exists(s) && Coveo.Utils.isNonEmptyString(Coveo.Utils.trim(s)) });
    }
    ...
    static emailAddressesToHyperlinks(addresses: string[], companyDomain?: string, me?: string, lengthLimit = 2, truncateName = false): string {
      addresses = _.filter(addresses, (s: string) => { return Coveo.Utils.exists(s) && Coveo.Utils.isNonEmptyString(Coveo.Utils.trim(s)) });
