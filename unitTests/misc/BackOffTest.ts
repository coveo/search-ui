import { backOff, IBackOffRequest, IBackOffOptions } from '../../src/misc/BackOff';
...
export function BackOffTest() {
  describe('BackOff', () => {
    ...
    let backOffOptions: IBackOffOptions;
    ...
    describe('when #BackOffRequest.fn is a promise that is rejected', () => {
      ...
      it('retries the request as many times as specified in #BackOffOptions.numOfAttempts', done => {
        ...
        request.catch(() => {
          expect(backOffRequest.fn).toHaveBeenCalledTimes(backOffOptions.numOfAttempts);
          done();
        });
      });
