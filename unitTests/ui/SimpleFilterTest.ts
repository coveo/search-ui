...
export function SimpleFilterTest() {
  const enableClearButtonOptions = [false, true, null];
  ...
  describe('SimpleFilter', () => {
    ...
    enableClearButtonOptions.forEach(enableClearButton => {
      describe(`SimpleFilter core functionality with enableClearButton as ${enableClearButton}`, () => {
        beforeEach(() => {
        ...
        it('should handle the backdrop correctly', () => {
          ...
          aSimpleFilter.cmp.closeContainer();
          expect($$($$(aSimpleFilter.cmp.root).find('.coveo-dropdown-background')).hasClass('coveo-dropdown-background-active')).toBe(
            false
          );
        });
        ...
        it('should not render a clear button if enableClearButton is falsy', () => {
          expect($$(aSimpleFilter.cmp.element).findAll('.coveo-simplefilter-eraser').length).toBe(
            aSimpleFilter.cmp.options.enableClearButton ? 1 : 0
          );
        });
      });

      describe(`SimpleFilter with more than one instance and enabledClearButton as ${enableClearButton}`, () => {
        ...
        beforeEach(() => {
          anotherSimpleFilter = Mock.advancedComponentSetup<SimpleFilter>(
            SimpleFilter,
            new Mock.AdvancedComponentSetupOptions(null, simpleFilterOptions, (builder: Mock.MockEnvironmentBuilder) => {
    ...
    describe('SimpleFilter clear button functionality', () => {
      let simpleFilterWithEraser: Mock.IBasicComponentSetup<SimpleFilter>;
      let $$simpleFilterWithEraser: Dom;

      function clickOnClearButton() {
        $$($$simpleFilterWithEraser.find('.coveo-simplefilter-eraser')).trigger('click');
      }
      ...
      function getClearButton() {
        return $$($$simpleFilterWithEraser.find('.coveo-simplefilter-eraser'));
      }
      ...
      beforeEach(() => {
        ...
        $$simpleFilterWithEraser = $$(simpleFilterWithEraser.cmp.element);
