...
export function ComponentOptionsValidatorTest() {
  describe('ComponentOptionsValidator', () => {
    it('should not modify the dictionnary if the option has no validator', () => {
      const out = { foo: 'bar' };
      new ComponentOptionsValidator({}, { componentID: 'id', name: 'foo', value: 'bar' }, out).validate();
      expect(out.foo).toBe('bar');
    });

    it('should modify the dictionnary if the option has an invalid validator', () => {
      const out = { foo: 'bar' };
    ...
    it('should not modify the dictionnary if the option has a valid validator', () => {
      const out = { foo: 'bar' };
